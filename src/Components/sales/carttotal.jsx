...
const CartTotal = () => {
    ...
    // Function to handle printing of a specific cart
    const handlePrintCart = (cartId) => {
        const cartDetails = sales.find(cart => cart.id === cartId);
        if (cartDetails) {
            const printableContent = `
        ID: ${cartDetails.id}\n
        ${Object.values(cartDetails).map(item => `${item.name} - Ksh ${item.price} - Quantity: ${item.quantity}`).join('\n')}
        \nTotal Price: Ksh ${calculateCartTotal(cartDetails)}
      `;
            ...
            <body>
              <div class="container">
                <div class="cart-details">
                  ...
                  ${Object.values(cartDetails).map(item => `<p>${item.name} - Ksh ${item.price} - Quantity: ${item.quantity}</p>`).join('\n')}
